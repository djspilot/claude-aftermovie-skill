...
Three Adapters ship today:
  ...
  * `GoProICCAdapter` — one instance per HERO/GoPro reached over MTP via
    Apple's ImageCaptureCore framework (PyObjC). Covers HERO9 / HERO10
    which default to MTP and never mount as Mass Storage.

All three fulfil the `ImportSource` Protocol so the CLI, the future GUI,
and a future HTTP endpoint can iterate `all_sources()` and treat each
Adapter uniformly. New devices (DJI drone SD card, Android MTP) bolt on by
adding a new file under this package and prepending the registry tuple —
no churn in the call sites.
"""
